        ...
        <div className={'space-y-2 mt-6'}>
            {uploads.map(([name, file]) => (
                <div key={name} className={'flex items-center space-x-3 bg-zinc-700 p-3 rounded'}>
                    ...
                    >
                        FixMe: Agregar icono de X para cancelar carga
                    </button>
                </div>
            ))}
            <Dialog.Footer>
                ...
                <Button.Text onClick={close}>Cerrar</Button.Text>
            </Dialog.Footer>
        </div>
    );
};
...
const FileUploadListDialog = asDialog({
    title: 'Cargas de archivo',
    description: 'Los siguientes archivos se están cargando a su servidor.',
})(FileUploadList);
