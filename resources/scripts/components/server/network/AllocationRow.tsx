...
// Componente que representa una fila de asignación de red en la interfaz
const AllocationRow = ({ allocation }: Props) => {
    ...
    // Función que maneja el cambio de notas de la asignación
    const onNotesChanged = useCallback((id: number, notes: string) => {
        mutate((data) => data?.map((a) => (a.id === id ? { ...a, notes } : a)), false);
    }, []);

    // Función que actualiza las notas de la asignación con debounce
    ...
    // Función que establece la asignación como primaria
            ...
            <div className={'flex items-center w-full md:w-auto'}>
                <div className={'mr-4 flex-1 md:w-40'}>
                    ...
                    <label className='uppercase text-xs mt-1 text-zinc-400 block px-1 select-none transition-colors duration-150'>
                        {allocation.alias ? 'Nombre de host' : 'Dirección IP'}
                    </label>
                </div>
