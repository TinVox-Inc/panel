...
const schema = object().shape({
    ...
    payload: string().when('action', {
        is: (v) => v !== 'backup',
        then: () => string().required('Se debe proporcionar una carga útil de la tarea.'),
        otherwise: () => string(),
    }),
    continueOnFailure: boolean(),
    timeOffset: number()
        .typeError('El desplazamiento de tiempo debe ser un número válido entre 0 y 900.')
        .required('Se debe proporcionar un valor de desplazamiento de tiempo.')
        .min(0, 'La compensación de tiempo debe ser al menos 0 segundos.')
        .max(900, 'El desplazamiento de tiempo debe ser inferior a 900 segundos.'),
});
...
const TaskDetailsModal = ({ schedule, task }: Props) => {
    ...
    const submit = (values: Values, { setSubmitting }: FormikHelpers<Values>) => {
        ...
        if (backupLimit === 0 && values.action === 'backup') {
            ...
            addError({
                message:
                    'No se puede crear una tarea de copia de seguridad cuando el límite de copia de seguridad del servidor se establece en 0.',
                key: 'schedule:task',
            });
        } else {
                    ...
                    <Form>
                        ...
                        <div className={`flex flex-col gap-3`}>
                            ...
                            <div>
                                <Field
                                    name={'timeOffset'}
                                    label={'Desplazamiento de tiempo (en segundos)'}
                                    description={
                                        'La cantidad de tiempo para esperar después de que la tarea anterior se ejecute antes de ejecutar esta.Si esta es la primera tarea en un horario, esto no se aplicará.'
                                    }
                                />
                            </div>
                        </div>
                        <div className={`my-6`}>
                            {values.action === 'command' ? (
                                ...
                                <div>
                                    <Label>Archivos ignorados (opcionales)</Label>
                                    <FormikFieldWrapper
                                        name={'payload'}
                                        description={
                                            'Incluya los archivos y carpetas que se excluirán en esta copia de seguridad.Por defecto, se utilizará el contenido de su archivo .peroignore.Si ha alcanzado su límite de respaldo, la copia de seguridad más antigua se girará.'
                                        }
                                    >
                                        <FormikField
                        ...
                        <FormikSwitchV2
                            ...
                            description={'Se ejecutarán tareas futuras si esta tarea falla.'}
                            label={'Continuar con el fracaso'}
                        />
