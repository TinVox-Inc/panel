...
const StartupContainer = () => {
    ...
    return !data ? (
        !error || (error && isValidating) ? (
            ...
            <ServerError title={'Ups!'} message={httpErrorToHuman(error)} />
        )
    ) : (
        <ServerContentBlock title={'Startup Settings'} showFlashKey={'startup:image'}>
            <MainPageHeader direction='column' title='Startup Settings'>
                <h2 className='skeleton-anim-4 text-sm'>
                    Estas configuraciones se utilizan para controlar cómo se inicia su servidor. Tenga cuidado al
                    modificar estos Configuración, ya que pueden hacer que su servidor se vuelva inoperable.
                </h2>
            </MainPageHeader>
            <div className={`skeleton-anim-4 grid lg:grid-cols-3 gap-8`}>
                ...
                <TitledGreyBox title={'Imagen de Docker'}>
                    {Object.keys(data.dockerImages).length > 1 && !isCustomImage ? (
                        <>
                            ...
                            <p className={`text-xs mt-2`}>
                                Esta es una característica avanzada que le permite seleccionar una imagen de Docker para
                                usar cuando se ejecuta esta instancia del servidor.
                            </p>
                        </>
                    ) : (
                        <>
                            ...
                            {isCustomImage && (
                                <p className={`text-xs text-zinc-300 mt-2`}>
                                    Esta imagen de Docker del servidor ha sido establecida manualmente por un
                                    administrador y no puede ser cambiado a través de esta interfaz de usuario.
                                </p>
                            )}
                        </>
                    )}
            ...
            {data && (
                <>
                    <h3 className={`skeleton-anim-4 mt-8 mb-4 text-2xl font-extrabold`}>Variables</h3>
                    <div className='skeleton-anim-4 h-[47svh] flex flex-col justify-between'>
